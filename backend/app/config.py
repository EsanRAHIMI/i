...
class Settings(BaseSettings):
    ...
    ALLOWED_ORIGINS: List[str] = [
        ...
        "http://localhost:8000",
        "http://localhost",
        # اگر فرانت روی دامنه است، این‌ها را باز کن/بروزرسانی کن:
        # "https://aidepartment.net",
        # "https://app.aidepartment.net",
    ]

    # TrustedHost (Starlette): فقط دامنهٔ دقیق یا وایلدکارت به شکل *.example.com
    # پورت/پروتکل ننویس؛ * تنها مجاز نیست.
    ALLOWED_HOSTS: List[str] = [
        "localhost",
        "127.0.0.1",
        "api.aidepartment.net",
        "*.aidepartment.net",  # نگه‌دار اگر احتمال ساب‌دامین‌های دیگر هست
        # "0.0.0.0",  # معمولاً در Host header نمی‌آید؛ می‌توانی حذفش کنی
    ]
    ...
    def parse_allowed_hosts(cls, v):
        # اجازه نده پروتکل/پورت/الگوهای نامعتبر وارد شوند
        def _clean(item: str) -> Optional[str]:
            s = item.strip()
            if not s:
                return None
            # حذف protocol و port اگر اشتباهی آمده باشد
            if "://" in s:
                s = s.split("://", 1)[1]
            if ":" in s:
                # Starlette پورت را در الگو نمی‌پذیرد
                s = s.split(":", 1)[0]
            # ممنوع: '*' تنها یا الگوهای غیر از '*.domain.tld'
